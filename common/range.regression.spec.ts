...
describe('Range selection', () => {
  ...
  it('marks the band between both ends inclusively', () => {
    ...
    expect(utils.getRangeState(moment('2026-09-10'), selected).isStart).toBeTrue();
    expect(utils.getRangeState(moment('2026-09-14'), selected).isEnd).toBeTrue();
    expect(utils.getRangeState(moment('2026-09-12'), selected).isInRange).toBeTrue();
    expect(utils.getRangeState(moment('2026-09-15'), selected).isInRange).toBeFalse();
  });
  ...
  it('previews the band up to the hovered day while the range is half open', () => {
    ...
    expect(preview.isInRange).toBeTrue();
    expect(preview.isPreview).toBeTrue();
    ...
    expect(closed.isPreview).toBeFalse();
  });
  ...
  it('previews backwards when hovering before the open start', () => {
    ...
    expect(state.isInRange).toBeTrue();
    expect(state.isStart).toBeFalse();
  });
...
describe('Action bar defaults', () => {
  ...
  it('keeps single day selection auto-committing with no confirm button', () => {
    ...
    expect(config.showActionButtons).toBeFalse();
    expect(config.closeOnSelect).toBeTrue();
  });

  it('shows the bar and stops auto-closing for range, time and daytime', () => {
    expect(service.getConfig({selectionMode: 'range'}, 'day').showActionButtons).toBeTrue();
    expect(service.getConfig({selectionMode: 'range'}, 'day').closeOnSelect).toBeFalse();
    expect(service.getConfig({}, 'time').showActionButtons).toBeTrue();
    expect(service.getConfig({}, 'daytime').showActionButtons).toBeTrue();
  });

  it('lets the host override the automatic decision', () => {
    expect(service.getConfig({showActionButtons: false}, 'daytime').showActionButtons).toBeFalse();
    expect(service.getConfig({showActionButtons: true}, 'day').showActionButtons).toBeTrue();
    expect(service.getConfig({showActionButtons: true}, 'day').closeOnSelect).toBeFalse();
  });
});
